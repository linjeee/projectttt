...
if(isset($_POST['like_content'])){
   ...
   if($user_id != ''){
      ...
      if($select_likes->rowCount() > 0){
         ...
         $insert_likes = $conn->prepare("INSERT INTO `likes`(user_id, tutor_id, post_id) VALUES(?,?,?)");
         $insert_likes->execute([$user_id, $tutor_id, $post_id]);
...
if(isset($_POST['add_comment'])){
   ...
   if($user_id != ''){
      ...
      if($select_content->rowCount() > 0){
         ...
         $select_comment = $conn->prepare("SELECT * FROM `comments` WHERE post_id = ? AND user_id = ? AND tutor_id = ? AND comment = ?");
         $select_comment->execute([$post_id, $user_id, $tutor_id, $comment_box]);

         if($select_comment->rowCount() > 0){
            $message[] = 'comment already added!';
         }else{
            $insert_comment = $conn->prepare("INSERT INTO `comments`(id, post_id, user_id, tutor_id, comment) VALUES(?,?,?,?,?)");
            $insert_comment->execute([$id, $post_id, $user_id, $tutor_id, $comment_box]);
...
<section class="watch-video">
   ...
   <?php
      $select_content = $conn->prepare("SELECT * FROM `post` WHERE id = ? AND status = ?");
   ...
   <div class="video-details">
      <img src="uploaded_files/<?= $fetch_content['thumb']; ?>" class="video" poster="uploaded_files/<?= $fetch_content['thumb']; ?>" controls autoplay></img>
